...
private:
	void ADD_RECORD(int, string, string, string, int, int, int, string, char, char, char, float, float);
	void MODIFY_RECORD(int, string, string, string, string, char, char, char, float, float);
...
void EMPLOYEE::ADD_RECORD(int ecode, string ename, string eaddress, string ephone, int d, int m, int y, string edesig, char egrade, char ehouse, char econv, float eloan, float ebasic)
{
	fstream file;
...
void EMPLOYEE::MODIFY_RECORD(int ecode, string ename, string eaddress, string ephone, string edesig, char egrade, char ehouse, char econv, float eloan, float ebasic)
{
	int recno;
...
void EMPLOYEE::NEW_EMPLOYEE(void)
{
	...
	if (ecode == 1)
	{
		ADD_RECORD(ecode, "", "", "", 0, 0, 0, "", 'n', 'n', 'n', 0.0, 0.0);
		DELETE_RECORD(ecode);
	}
	gotoxy(21, 5);
	cout << ecode;
	do
	{
		...
		getline(cin, ename);
		std::for_each(ename.begin(), ename.end(), [](char &c)
					  { c = ::toupper(c); });

		if (ename[0] == '0')
			return;
		if (ename.length() < 1 || ename.length() > 25)
		{
			valid = 0;
			gotoxy(5, 25);
			clreol();
	...
	do
	{
		...
		std::for_each(eaddress.begin(), eaddress.end(), [](char &c)
					  { c = ::toupper(c); });

		if (eaddress[0] == '0')
			return;
		if (eaddress.length() < 1 || eaddress.length() > 30)
		{
			valid = 0;
			gotoxy(5, 25);
			clreol();
	...
	do
	{
		...
		if (ephone[0] == '0')
			return;
		if ((ephone.length() < 7 && ephone.length() > 0) || (ephone.length() > 9))
		{
			valid = 0;
			gotoxy(5, 25);
			clreol();
	...
	do
	{
		...
		do
		{
			...
			clreol();
			getline(cin, tday);
			char s_tday[tday.length()];
			int i;
			for (i = 0; i < sizeof(s_tday); i++)
			{
				s_tday[i] = tday[i];
			}
		...
		do
		{
			...
			clreol();
			getline(cin, tmonth);
			char s_tmonth[tmonth.length()];
			int i;
			for (i = 0; i < sizeof(s_tmonth); i++)
			{
				s_tmonth[i] = tmonth[i];
			}
			td = atoi(s_tmonth);
		...
		do
		{
			...
			clreol();
			getline(cin, tyear);
			char s_tyear[tyear.length()];
			int i;
			for (i = 0; i < sizeof(s_tyear); i++)
			{
				s_tyear[i] = tyear[i];
			}
			td = atoi(s_tyear);
	...
	do
	{
		...
		clreol();
		getline(cin, edesig);
		std::for_each(edesig.begin(), edesig.end(), [](char &c)
					  { c = ::toupper(c); });

		if (edesig[0] == '0')
			return;
		if (edesig.length() < 1 || edesig.length() > 15)
		{
			valid = 0;
			gotoxy(5, 25);
			clreol();
